...
public partial class HistoryContextMenu : ContextMenuStrip
{
    ...
    {
        // 创建排序菜单项
        var sortMenuItem = new ToolStripMenuItem("排序");

        // 创建升序和降序菜单项
        var ascItem = new ToolStripMenuItem($"按{name}升序") { CheckOnClick = true };
        var descItem = new ToolStripMenuItem($"按{name}降序") { CheckOnClick = true };

        // 当前属性比较器优先级
        try
        {
            var priority = Methods.GetSortPriority(name);
    ...
    {
        // 创建筛选菜单项
        var filterMenuItem = new ToolStripMenuItem($"筛选");
        ...
        if (name == "难度")
        {
            ...
            {
                // 格式化难度名称
                var difficultyName = Methods.GetDifficultyText(difficulty);
