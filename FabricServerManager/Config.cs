...
namespace FabricServerManager
{
    ...
    public static class Config
    {
        ...
        {
            // 初始化配置数据为默认配置
            _configData = ConfigData.DefaultConfigData;
        ...
        private static void LoadConfig()
        {
            try
            {
                ...
                _configData = JsonSerializer.Deserialize<ConfigData>(json, Constants.JsonOptions) ?? ConfigData.DefaultConfigData;
        ...
        public static void SaveConfig()
        {
            ...
            try
            {
                var json = JsonSerializer.Serialize(_configData, Constants.JsonOptions);
        ...
        public static void HandleConfigCommand(string input)
        {
            ...
            switch (command)
            {
                ...
                case var cmd when cmd.StartsWith(WaitMinutesCommand, StringComparison.OrdinalIgnoreCase):
                    if (hasValue)
                    {
                        if (float.TryParse(value, out float waitMinutes) && waitMinutes > 0)
                        {
                ...
                case var cmd when cmd.StartsWith(CommandExecutionInfoTimeCommand, StringComparison.OrdinalIgnoreCase):
                    if (hasValue)
                    {
                        if (int.TryParse(value, out int executionTime) && executionTime > 0)
                        {
