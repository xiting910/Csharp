...
namespace FabricServerManager
{
    ...
    public static partial class Run
    {
        ...
        /// 玩家数量增加
        /// </summary>
        private static void IncrementPlayerCount() => Interlocked.Increment(ref _playerCount);
        /// <summary>
        /// 玩家数量减少
        /// </summary>
        private static void DecrementPlayerCount() => Interlocked.Decrement(ref _playerCount);
        /// <summary>
        /// 自动关闭服务器功能切换
        /// </summary>
            ...
            while (!process.HasExited)
            {
                try
                {
                    ...
                    else if (input.Trim().Equals("restart", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        _restartServer = true;
                        process.StandardInput.WriteLineAsync(Constants.StopCommand);
                    }
        ...
        public static bool RunServer(string serverPath)
        {
            ...
            string serverFile = Path.GetFileName(serverPath);
            string ServerFolder = Path.GetDirectoryName(serverPath) ?? throw new Exception("无法获取服务器文件夹路径");
            ...
            {
                // 构建启动命令
                string command = $"java -Xms{Config.Data.JavaXms} -Xmx{Config.Data.JavaXmx} -jar \"{serverFile}\" nogui";
                ...
                // 启动服务器进程
                using Process process = Process.Start(startInfo) ?? throw new Exception("无法启动服务器进程");
                ...
                process.OutputDataReceived += (sender, e) =>
                {
                    if (e.Data != null && e.Data.Contains("- ") && !_serverStarted)
                    {
                        if (e.Data.Trim().Split("- ")[0].All(c => c == ' ') && !e.Data.Contains("-- "))
                        {
                            ...
                            {
                                // 提取Mod名称并添加到ModList
                                string modName = e.Data.Trim().Split("- ")[1].Trim();
                ...
                process.OutputDataReceived += (sender, e) =>
                {
                    if (e.Data != null && e.Data.Contains(Constants.PlayerJoinMessage))
                    {
                        ...
                        string[] parts = e.Data.Split(Constants.PlayerJoinMessage, StringSplitOptions.RemoveEmptyEntries);
                        string playerName = parts[0].Split(' ', StringSplitOptions.RemoveEmptyEntries).Last();
                ...
                process.OutputDataReceived += (sender, e) =>
                {
                    if (e.Data != null && e.Data.Contains(Constants.PlayerLeaveMessage))
                    {
                        ...
                        string[] parts = e.Data.Split(Constants.PlayerLeaveMessage, StringSplitOptions.RemoveEmptyEntries);
                        string playerName = parts[0].Split(' ', StringSplitOptions.RemoveEmptyEntries).Last();
                        IOMethods.ShowCurrentTimestamp(ConsoleColor.Blue);
                ...
                process.OutputDataReceived += (sender, e) =>
                {
                    if (e.Data != null && e.Data.Contains(Constants.CommandMessage))
                    {
                        try
                        {
                            ...
                            string executor = e.Data[startIndex..endIndex];

                            startIndex = e.Data.IndexOf("[Command: /") + "[Command: /".Length;
                            endIndex = e.Data.IndexOf(']', startIndex);
                            string command = e.Data[startIndex..endIndex];

                            if (command.StartsWith(Constants.ChatMessage))
                            {
                                // 提取聊天消息内容
                                string chatMessage = command[Constants.ChatMessage.Length..].Trim();
                                // 输出聊天消息
                ...
                process.OutputDataReceived += (sender, e) =>
                {
                    if (e.Data != null && e.Data.Contains("There are") && e.Data.Contains("players online:"))
                    {
                        try
                        {
                            ...
                            if (match.Success)
                            {
                                int currentPlayers = int.Parse(match.Groups[1].Value);
                                int maxPlayers = int.Parse(match.Groups[2].Value);
                                string playersString = match.Groups[3].Value.Trim();
                                ...
                                {
                                    // 分割玩家名称
                                    string[] players = playersString.Split(", ", StringSplitOptions.RemoveEmptyEntries);
                ...
                while (!process.HasExited)
                {
                    ...
                    lock (_lastPlayerLeaveTimeLock)
                    {
                        ...
                        {
                            TimeSpan idleTime = DateTime.Now - _lastPlayerLeaveTime.Value;
