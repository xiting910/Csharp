        ...
        public static bool CheckJavaEnvironment()
        {
            ...
            try
            {
                ...
                var javaVersion = Process.Start(new ProcessStartInfo
                {
                    ...
                    CreateNoWindow = true
                }) ?? throw new Exception("Java未安装或未配置环境变量");
                javaVersion.WaitForExit();

                string output = javaVersion.StandardOutput.ReadToEnd().Trim();
                ...
                throw new Exception("无法获取Java版本信息");
            }
            catch (Exception ex)
            {
        ...
        public static string? FindServerFiles()
        {
            ...
            if (File.Exists(serverDataFile))
            {
                ...
                try
                {
                    ...
                    else if (serverData.FoundServerFiles.Count > 0)
                    {
                        ...
                        for (int i = 0; i < serverData.FoundServerFiles.Count; i++)
                        {
                        ...
                        if (int.TryParse(input, out int index) && index > 0 && index <= serverData.FoundServerFiles.Count)
                        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    string? input = Console.ReadLine();
                    if (int.TryParse(input, out int index) && index > 0 && index <= serverFiles.Count)
                    {
